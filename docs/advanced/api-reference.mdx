...
Load a prompt by name and optional version. If version is `null`, the [active version](/core/prompts#version-resolution-order) is resolved automatically. Returns a `PromptTemplate` instance.
...
`PromptPHP\Deck\Facades\PromptDeck`
...
`PromptPHP\Deck\Exceptions\PromptDeckException`
